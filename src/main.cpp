...
async function checkForUpdates(manual){const t=TR[LG];try{const res=await fetch('https://api.github.com/repos/'+FW_REPO+'/releases/latest',{headers:{'Accept':'application/vnd.github+json'}});if(!res.ok)throw new Error('github');const rel=await res.json();const latest=rel.tag_name||rel.name||'';if(cmpVer(latest,FW_VER)<=0){if(manual)alert(t.upToDate+' ('+FW_VER+')');return;}const asset=(rel.assets||[]).find(a=>String(a.name||'').toLowerCase().endsWith('.bin'));const openUrl=asset&&asset.browser_download_url?asset.browser_download_url:rel.html_url;if(!asset&&manual)alert(t.updateOpenRelease);if(confirm(fmt(t.updateAvail,{latest:latest,current:FW_VER}))){window.open(openUrl,'_blank','noopener');}}catch(e){if(manual)alert(t.updateError);}}
